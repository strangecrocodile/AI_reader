...
/** 演示模式使用的知识点聚合数据。 */
export function knowledgeFor(bookId) {
  ...
  for (const chapter of book?.chapters ?? []) {
    ...
    for (const point of content?.knowledgePoints ?? []) {
      concepts.push({
        ...
        status: chapter.isToday ? 'learning' : chapter.status === 'done' ? 'learned' : 'planned',
        mastery: chapter.isToday ? 42 : chapter.status === 'done' ? 100 : 0,
      });
    }
  }
