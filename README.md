...
**扫描件 PDF 报错？**
扫描件没有文本层，属于产品范围之外（见 `docs\产品设计文档.md` 第 6 节）。

**上传后提示「内容可能没被完整读取」？**
解析器读不全这本教材，提示里会写清**跳过了什么**。现在表格已经能读到了，
剩下最常见的是正文写在 **Word 文本框**里、或整本是**扫描图片**——前者 `doc.paragraphs`
读不到，后者一个字都提不出来。把内容改成普通段落（章节用「标题 1 / 标题 2」样式）
后重新上传最稳，或另存为 `.md` / `.txt`。
