...
> 状态：**草稿，待小组评审**（本文件已随 kb-agent 并入 AI_reader 仓库）。
...
> 历史说明：本文档原为仓库外草稿，现已随 kb-agent 并入本仓库 `tools/kb-agent`。
...
- ~~不推 GitHub、不改 AI_reader 版本历史（组员处理）~~ —— kb-agent 已并入 `AI_reader/tools/kb-agent`，本条已失效。
